...
/* 7.1 最终分析数据集样本量 */
proc sql;
    create table out.sample_size_summary as
    select
        count(*) as final_n,
        count(distinct SEQN) as unique_subjects,
        sum(death=1) as death_n,
        sum(death=0) as censored_n
    from out.nhanes_2011_2018_analysis;
quit;

/* 7.2 各 NHANES 周期样本量 */
proc freq data=out.nhanes_2011_2018_analysis;
    tables cycle / missing;
    ods output OneWayFreqs=out.sample_by_cycle;
run;
...
/* 7.4 关键连续变量缺失与分布 */
proc means data=out.nhanes_2011_2018_analysis
    n nmiss mean std min p25 median p75 max;
    var RIDAGEYR LBXTC LBDHDD LBXTR LBDLDL follow_month;
    ods output Summary=out.key_variable_summary;
run;

/* 7.5 按周期查看关键变量缺失情况 */
proc means data=out.nhanes_2011_2018_analysis n nmiss mean std min median max;
    class cycle;
    var RIDAGEYR LBXTC LBDHDD LBXTR LBDLDL follow_month;
run;

/* 7.6 导出数据完整性检查结果 */
proc export data=out.sample_size_summary
    outfile="&root/Kaplan-Meier/data_integrity_sample_size.xlsx"
    dbms=xlsx
    replace;
run;

proc export data=out.key_variable_summary
    outfile="&root/Kaplan-Meier/data_integrity_key_variables.xlsx"
    dbms=xlsx
    replace;
run;
